...
with WiFiCatcher so EAP method enumeration works out of the box (no separate
install). WiFiCatcher runs it from a temporary copy, so the files here are never
modified at runtime.
