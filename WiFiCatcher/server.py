...
async def _lifespan(app: FastAPI):
    yield
    # Graceful shutdown: stop live capture (kills airodump-ng, restores managed
    ...
    try:
        ...
        pass
    # Restart runs in the root warden, which only (re)starts NetworkManager when
    # it is down, so a session that never captured is left untouched.
    try:
...
def create_app() -> FastAPI:
    ...
    @app.get("/")
    def index():
        # no-cache so the browser revalidates and picks up the versioned
        # app.js / style.css links instead of a stale cached build.
        return FileResponse(
